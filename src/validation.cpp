...
#if defined(NDEBUG)
# error "Luso Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    int nFairLaunch = consensusParams.nFairLaunch;
    //LogPrintf("Reward pow step1: %d <? %d\r\n",nPrevHeight,nFairLaunch);
    if (nPrevHeight == 0) {
        return 10368000 * COIN;
    }
    if (nPrevHeight > 0 && nPrevHeight < nFairLaunch) { // fair launch block 14400 kick in PoW, predicted half month
        return 5 * COIN;
    }
    nPrevHeight-=nFairLaunch; // fix cycles calc
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue, std::string country)
{
    ...
    if (nHeight > Params().GetConsensus().nGEOLaunch) {
        ...
        if (geor.isLUSO(country.c_str(),(char*) country.c_str()) == 0) {
            cycledRewardPerc -= 0.15 * GetLusoShare(mnrounded); // Change reward for LUSO countries
        }
    }
    if (cycledRewardPerc>0.45)cycledRewardPerc=0.45;
    ret += blockValue * cycledRewardPerc; // start at 25%
...
bool IsInitialBlockDownload()
{
    ...
    if (chainActive.Tip() == NULL)
        return true;
    //if (UintToArith256(chainParams.GetConsensus().nMinimumChainWork) > 0 && chainActive.Tip()->nChainWork < UintToArith256(chainParams.GetConsensus().nMinimumChainWork))
    //    return true;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    LogPrintf("%s: invalid block=%s   height=%d (%d)  log2_work=%.8g  date=%s\n", __func__,
      pindexNew->GetBlockHash().ToString(), pindexNew->nHeight, chainActive.Height(),
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // LUSO : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(LUSO): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(LUSO): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    //if (block.GetHash() == uint256S("c27cfd618666b73c50c4e96fb2da326bf72523172554f2ae11beedef5750353b"))
    //    return true;
    if (block.fChecked)
        return true;
    ...
    // LUSO : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(LUSO): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(LUSO): spork is off, skipping transaction locking checks\n");
    }

    // END LUSO
    //printf("Verifying TX of block %s\n",block.GetHash().ToString().c_str());
    // Merkleblock fail sig bypass
    /*
    if (
      (fRequireStandard && block.GetHash() != uint256S(""))
      &&
      (!fRequireStandard && block.GetHash() != uint256S("0000049621362b83e85a8f3978815691059bdd046c784139277135c2212bd414"))
      &&
      (!fRequireStandard && block.GetHash() != uint256S("0000049621362b83e85a8f3978815691059bdd046c784139277135c2212bd414"))
      ) {
    */
...
static bool AcceptBlock(const CBlock& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock)
{
    ...
    //if (pindex->nHeight > 0 && ((!CheckBlock(block, state)) || !ContextualCheckBlock(block, state, pindex->pprev))) {
