...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "luso" is a composite category enabling all Luso-related debug output
            if(ptrCategory->count(string("luso"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LusoCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LusoCore
    // Mac: ~/Library/Application Support/LusoCore
    // Unix: ~/.lusocore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LusoCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/LusoCore";
#else
    // Unix
    return pathRet / ".lusocore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    {
        // Don't overwrite existing settings so command line settings override luso.conf
