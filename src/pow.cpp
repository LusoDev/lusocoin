...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const Consensus::Params& params) {
    /* current difficulty formula, luso - DarkGravity v3, written by Evan Duffield - evan@luso.org */
    const arith_uint256 bnPowLimit = UintToArith256(params.powLimit);
    int64_t nPastBlocks = 10;
...
unsigned int GetNextWorkRequiredBTC(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
		...
		if (pindexLast == NULL)
			return pindexLast->nBits; // genesis block

		if (pindexLast->pprev == NULL)
			return pindexLast->nBits; // first block

		const CBlockIndex* pindexPrevPrev = pindexLast->pprev;
		if (pindexPrevPrev->pprev == NULL)
			return pindexLast->nBits; // second block

        double ratio=1;
        if (pindexLast->nHeight < 1000)ratio=0.05;
        else if (pindexLast->nHeight < 5000)ratio=0.1;
        else if (pindexLast->nHeight < 10000)ratio=0.3;
        else if (pindexLast->nHeight < 20000)ratio=0.6;
        // one day retarget after block 20 000

        int64_t nTargetSpacing = params.nPowTargetTimespan * ratio;
		int64_t nActualSpacing = pindexLast->GetBlockTime() - pindexPrevPrev->GetBlockTime();
        //printf("Spacing+ratio: %i (%i, %i), Actual: %i\n",nTargetSpacing, nTargetSpacing*2, nTargetSpacing/3, nActualSpacing);
		if (nActualSpacing < 0) {
