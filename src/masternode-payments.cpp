...
*
*   Why is this needed?
*   - In Luso some blocks are superblocks, which output much higher amounts of coins
...
bool IsBlockValueValid(const CBlock& block, int nBlockHeight, CAmount blockReward, std::string &strErrorRet)
{
    ...
    int nBlockHeightfix = nBlockHeight-consensusParams.nFairLaunch;
    if (nBlockHeightfix<0)nBlockHeightfix=nBlockHeight;
...
{
    if(fLiteMode) return; // disable all Luso specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f LUSO\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
