...
pkgdatadir = $(datadir)/lusocore
pkgincludedir = $(includedir)/lusocore
pkglibdir = $(libdir)/lusocore
pkglibexecdir = $(libexecdir)/lusocore
...
CONFIG_HEADER = $(top_builddir)/src/config/luso-config.h
CONFIG_CLEAN_FILES = liblusoconsensus.pc share/setup.nsi \
...
pkgconfig_DATA = liblusoconsensus.pc
BITCOIND_BIN = $(top_builddir)/src/lusod$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/luso-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/luso-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
OSX_APP = Luso-Qt.app
OSX_DMG = Luso-Core.dmg
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_luso_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_luso_coverage.info test_luso.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Luso-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/luso-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/luso-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/luso-config.h
$(top_srcdir)/src/config/luso-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/luso-config.h src/config/stamp-h1
liblusoconsensus.pc: $(top_builddir)/config.status $(srcdir)/liblusoconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/Luso-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Luso-Qt

$(OSX_DMG): $(APP_DIST_EXTRAS)
	$(GENISOIMAGE) -no-cache-inodes -D -l -probe -V "Luso-Core" -no-pad -r -dir-mode 0755 -apple -o $@ dist
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Luso-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_luso.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_luso -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_luso_filtered.info: test_luso.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_luso_filtered.info
#	$(MKDIR_P) qa/tmp
...
#rpc_test.info: test_luso_filtered.info
...
#test_luso_coverage.info: baseline_filtered_combined.info test_luso_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_luso_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_luso_filtered.info block_test_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_luso_filtered.info -a block_test_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_luso.coverage/.dirstamp:  test_luso_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_luso.coverage/.dirstamp total.coverage/.dirstamp
...
clean-local:
	rm -rf coverage_percent.txt test_luso.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
